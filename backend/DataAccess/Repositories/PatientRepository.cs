    ...
    public class PatientRepository : IPatientRepository
    {
        ...
        public async Task<Patient?> GetByIdAsync(int id)
        {
            using (var connection = _context.Connect())
            {
                ...
                string sql =
                    "SELECT p.*, e.Id, e.DateTime, e.Anamnesis, e.Motivation, e.Category, e.patientId FROM Patients p "
                    + "INNER JOIN Examinations e ON p.Id = e.PatientId "
                    + "WHERE PatientId = @ID";

                await connection.QueryAsync<Patient, Examination, Patient>(
                    sql,
                    (patient, examination) =>
                    {
                        if (result == null)
        ...
        public async Task CreateAsync(Patient patient)
        {
            using (var connection = _context.Connect())
            {
                ...
                string sql =
                    "INSERT INTO Patients (FirstName, LastName, Gender, FiscalCode, Photo, DateOfBirth, CreatedAt) "
